...
- 🔗 **자동 URL 수집 & 비동기 처리**  
  URL을 자동 수집하고, 백그라운드에서 콘텐츠 분석을 실행해 빠르고 끊김 없는 사용 경험 제공.

- 🤖 **AI 기반 자동 카테고리 분류**  
  콘텐츠 의미를 분석해 주제를 자동 생성하고 정리.

- 📝 **스마트 요약 & 태그 생성**  
  핵심 3~5줄 요약과 관련 태그를 자동으로 생성하여 빠르게 내용 파악.

- 🔍 **의미론적 검색(Semantic Search)**  
  단순 키워드가 아닌 의미 기반 검색으로 더 정확한 결과 제공.

- 💬 **AI 지식 탐색 에이전트**  
  저장된 콘텐츠를 기반으로 질문에 응답하는 대화형 에이전트 제공.

- 📊 **활동 대시보드 & 자동 브리핑**  
  학습 패턴 시각화 + APScheduler 기반 자동/수동 브리핑 기능 지원.

- 🔒 **프라이버시 중심 구조**  
  모든 데이터(콘텐츠, SQLite, ChromaDB)는 **로컬에서만 처리**, 개인 정보 보호 강화.
